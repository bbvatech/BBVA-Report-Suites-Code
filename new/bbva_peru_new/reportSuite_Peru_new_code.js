...
if (_satellite && _satellite.settings.isStaging == true)
    ...
    r_suite = 'bbvap.global.2016.prod'; //dev suite
...
s.visitor = Visitor.getInstance("FB99EDA0570E88407F000101@AdobeOrg",{
    trackingServer: "bbvape.d3.sc.omtrdc.net"
});
...
s.forcedLinkTrackingTimeout = 500; //Tiempo de espera para lanzar tl
...
s.loadModule("Media");
s.eVar99 = _satellite.getVar("translate cid");
...
function onYouTubeIframeAPIReadyDTM() {
    ...
    $('iframe[id*="player"').each(function(index) {
        ...
        oPlayer[num_videos] = new YT.Player(idPlayer, {
            events: {
...
function onPlayerReadyDTM(event) {
    ...
    // console.log('*** iFrame embed onPlayerReadyDTM ', oPlayer[num]);
}
...
s.handlePPVevents = function() {
    ...
    var dh = Math.max(Math.max(s.d.body!= null ? s.d.body.scrollHeight:0, s.d.documentElement.scrollHeight), Math.max(s.d.body != null ? s.d.body.offsetHeight:0, s.d.documentElement.offsetHeight), Math.max(s.d.body != null ?s.d.body.clientHeight:0, s.d.documentElement.clientHeight)),
        vph = window.innerHeight || (s.d.documentElement.clientHeight || s.d.body != null ? s.d.body.clientHeight:0),
        st = window.pageYOffset || (window.document.documentElement.scrollTop || window.document.body != null ? window.document.body.scrollTop:0),
        vh = st + vph,
        pv = Math.min(Math.round(vh / dh * 100), 100),
        c = "";
    if (!s.c_r("tp") || decodeURIComponent(s.c_r("s_ppv").split(",")[0]) != s.getPPVid || s.ppvChange == "1" && (s.c_r("tp") && dh != s.c_r("tp"))) { s.c_w("tp", dh);
        s.c_w("s_ppv", "") } else c = s.c_r("s_ppv");
...
s.getPercentPageViewed = function(pid, change) {
    ...
    if (!s.getPPVid || s.getPPVid != pid) { s.getPPVid = pid;
    ...
    if (ist)
        ...
        } else if (window.attachEvent) { window.attachEvent("onload", s.handlePPVevents);
