...
function getDesElemento(obj) {
    var txtElemento = obj.innerHTML;
    txtElemento = txtElemento.toLowerCase();
    if (txtElemento.indexOf("<small>") > -1)
        txtElemento = txtElemento.substr(0, txtElemento.indexOf("<small>"));
    return parseaElemento(txtElemento);
}

function getDesElementoPPrinc(obj) {
    var txtElemento = obj.textContent;
    txtElemento = txtElemento.toLowerCase();
    return parseaElemento(txtElemento);
}
...
_satellite.setVar("etiquetaHome", pagina + "banner lateral:" + cadena);
return true;