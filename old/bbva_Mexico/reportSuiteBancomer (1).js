...
var s_account = "bbvabancomerdev,bbvabancomer";
...
s.linkInternalFilters = "javascript:,bancomer.com,//195.55.73.251/,tridion-portal.mex.igrupobbva" //optional: add your internal domain here
...
function s_doPlugins(s) {
    ...
    s.eVar69 = getCategoriaDeProducto(s.pageName);
}
...
/*
 * Plugin: getTrackingCode 1.0 (by Carlos Pliego)
 */

function getTrackingCode() {
    var res = "";
    var dis = s.getQueryParam("dis");
    var eml = s.getQueryParam("eml");
    var sem = s.getQueryParam("sem");
    var pdf = s.getQueryParam("pdf");
    var bid = s.getQueryParam("bid");
    var ons = s.getQueryParam("ons");
    var nwl = s.getQueryParam("nwl");
    var afl = s.getQueryParam("afl");
    var psm = s.getQueryParam("psm");
    ...
    if (dis.length > 0) {
        ...
        res = ons;
    } else if (nwl.length > 0) {
        res = nwl;
    } else if (afl.length > 0) {
        res = afl;
    } else if (psm.length > 0) {
        res = psm;
    }
    return res;
}
...
/*
 * Plugin: getCategoriaDeProducto 1.0 (by Carlos Pliego)
 */
function getCategoriaDeProducto(pageName) {
    //PROP2 CORRECTO => publica:personas:tarjetas:
    var res = "";
    var prop2 = "";
    var cont = 0;
